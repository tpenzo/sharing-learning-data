...
function PostItem({ dataItem, funcLikePost, funcUnLikePost, funcBookmarkPost, funcUnBookmarkPost, updateStatus }) {
            ...
            <Link to={`/profile/${dataItem?.author?._id}`}>
              <p className="font-semibold">
                {dataItem?.author?.fullName +
                  " " +
                  `${dataItem.author?.studentCode ? dataItem?.author?.studentCode : dataItem?.author?.teacherCode}`}
              </p>
            </Link>
            <span className="font-light text-gray-500 text-xs">{moment(dataItem?.createdAt).fromNow()}</span>
          </div>
        </div>
      ...
      <div className="flex items-end">
        {dataItem?.status === "posted" ? (
          <div className="w-full mt-5 flex items-center gap-5 md:gap-2">
            ...
            >
              <box-icon name="heart" type={loved ? "solid" : "regular"} color={loved ? "red" : "black"}></box-icon>
            ...
            <div className="relative">
              {options && (
                <ul className="absolute top-12 mt-1 right-0 w-36 bg-white shadow-2xl rounded-xl flex flex-col items-start justify-start overflow-hidden z-50">
                  ...
                  <ModalInstance
                    ...
                    modalBody={<FormPost onClose={onClose} isEdit={true} post={dataItem} />}
                  />
                  <li
                    className="w-full px-3 py-2 font-semibold cursor-pointer flex items-center justify-between hover:bg-gray-200"
              ...
              {dataItem?.author?._id === user._id && (
                <span
                  className={`cursor-pointer flex items-center rounded-lg p-1 ${
                    options ? "bg-gray-200" : "bg-inherit"
                  }`}
                  onClick={() => {
                    setOptions(!options);
                  }}
