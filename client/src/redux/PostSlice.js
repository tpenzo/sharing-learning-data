...
const PostSlice = createSlice({
  ...
  reducers: {
    createNewPost: (state, actions) => {
      actions.payload.data.status === "posted" && state.postList.unshift(actions.payload.data);
    },
      ...
      const updatedComments = state.commentsPostItem.map((item) => {
        if (item._id === cmtDadId) {
          return {
            ...item,
            reply: [
              ...item.reply.map((item) => (item._id === cmtId ? { ...item, likes: [...item.likes, userId] } : item)),
            ],
          };
        }
        return item;
    ...
    updateReplyComment: (state, actions) => {
      ...
      const updatedComments = state.commentsPostItem.map((item) => {
        if (item._id === cmtDadId) {
          return {
            ...item,
            reply: [...item.reply.map((item) => (item._id === cmtId ? { ...item, content: newContent } : item))],
          };
        }
        return item;
      });
    ...
    removePostCourseList: (state, actions) => {
      state.postCourseList = state.postCourseList.filter((post) => post._id !== actions.payload);
    },
  },
});
